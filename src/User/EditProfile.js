...
class EditProfile extends Component {
  ...
  isValid() {
    ...
    if (fileSize > 1000000) {
      //this size is in bytes (1000 kb limit or 1mb)
      this.setState({
    ...
    if (this.isValid()) {
      ...
      update(userId, token, this.userData).then((data) => {
        if (data.error) this.setState({ error: data.error, loading: false });
          ...
          // console.log(data);
          updateUserName(data, () => {
            //this is what passed to next parameter of updateUserName function in apiUser
  ...
  editForm(id, name, email, about, loading, photoUrl, error) {
  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        {this.editForm(id, name, email, about, loading, photoUrl, error)}
      </div>
    );
  }
}
