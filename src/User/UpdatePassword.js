...
class UpdatePassword extends Component{
    ...
    isValid(){  //check for errors if any while updating user
        ...
        const {currentPassword,currentPasswordVerify,hashed_password} = this.state;
        ...
        if(!currentPasswordVerify) {
            this.setState({ error : "Verify Current Password cannot be empty" });
            return false;
        }

        if(currentPassword !== currentPasswordVerify) {
            this.setState({ error : "Current Password and Verify Current Password must be same!!" });
            return false;
        }
        ...
        if(hashed_password.length > 0){
            ...
            this.setState({ error : "Password must contain atleast 8 characters!!" });
            return false;
            }

            if(!/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])/.test(hashed_password))
            {
                this.setState({ error : "Password must contain atlest 1 lowercase character , 1 uppercase character , 1 numeric character , 1 special character!!" });
                return false;   
            }
    ...
    clickSubmit = (event) => {
        ...
        if(this.isValid())
        {
            const user = {   
               ...
               // console.log(user);
                updatePassword(userId,token,user)
                .then( (data) => {
    ...
    editForm(id,currentPassword,hashed_password,currentPasswordVerify){ //edit form also takes image and accepts every type of image
        ...
        <form>
            <div className = "form-group">
                ...
                <input value = {currentPassword} onChange = {this.handleChange("currentPassword")} type = "password" className = "form-control" required/>
            </div>
            <div className = "form-group">
                <label className = "text-muted">Verfiy Current Password</label>
                <input value = {currentPasswordVerify} onChange = {this.handleChange("currentPasswordVerify")} type = "password" className = "form-control" required/>
            </div>
            <div className = "form-group">
                <label className = "text-muted">New Password (Please update this field if you want to update password)</label>
                <input value = {hashed_password} onChange = {this.handleChange("hashed_password")} type = "password" className = "form-control"/>
            </div>

            <div className = 'd-inline-block'>
                <button onClick = {this.clickSubmit} className = "btn btn-raised btn-primary">Update</button>
                <Link to = {`/user/${id}`} className = 'btn btn-raised btn-primary ml-5'>Cancel</Link>
            </div>
    ...
    render() {
        const {id,currentPassword,hashed_password,error,redirectToProfile,currentPasswordVerify} = this.state;
        ...
        return (
            <div className="container">
                ...
                {this.editForm(id,currentPassword,hashed_password,currentPasswordVerify)}
            </div>
        );
    }
