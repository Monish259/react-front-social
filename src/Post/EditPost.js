...
class EditPost extends Component{
    ...
    init = (postId) => {
        ...
        {
            if(data.error) this.setState( {redirectToHome : true} ); //if error in data then user is not authenticated and need to redirect it to signin page
            else {
                this.setState({ 
                    id : data._id,
                    title : data.title,
    ...
    isValid(){  //check for errors if any whil eupdating user
        ...
        if(title.length === 0 || title.length < 4) {
            this.setState({ error : "title is required of minimum length of 4" , loading : false});
            return false;
        }

        if(body.length === 0 || body.length < 4){  //check email domain with given regex
            this.setState({ error : "body is required of minimum length of 4" , loading : false});       
            return false;
        }
    ...
    editPostForm(id,title,body){ //edit form also takes image and accepts every type of image
        ...
        <form>
            ...
            <div className = 'd-inline-block'>
                ...
                <Link to = {`/`} className = 'btn btn-raised btn-primary ml-5'>Cancel</Link>
            </div>
    ...
    render() {
        ...
        const photoUrl = id ? `${process.env.REACT_APP_API_URL}/post/photo/${id}?${new Date().getTime()}` : postImage;
        ...
        return (
            <div className="container">
                ...
                onError = { (photo) => {return photo.target.src = `${postImage}`} } alt={`${title}`}/> 
