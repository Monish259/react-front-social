...
class SinglePost extends Component {
  ...
  likeToggle = () => {
    if (!isAuthenticated()) {
      this.setState({ error: "Please login to like this post" });
      return false;
    }
    ...
    if (this.state.like) {
      unLike(postId, userId, token).then((data) => {
        if (data.error) console.log(data.error);
        else
          this.setState({
      ...
      like(postId, userId, token).then((data) => {
        if (data.error) console.log(data.error);
        else
          this.setState({
            like: !this.state.like,
  ...
  deleteConfirmed = (postId, token) => {
    ...
    deletePost(postId, token).then((data) => {
      if (data.error) console.log("ERROR");
        ...
        this.setState({ redirectToHome: true, deleteLoading: false });
      }
    });
  };
  ...
  renderPost(post, error) {
    const { like, likes, likeLoading, deleteLoading } = this.state;
        ...
        return (
          <div className="card-body">
            ...
            {likeLoading ? (
              ...
              <h4 className="mb-4" onClick={this.likeToggle}>
                <i
              ...
              isAuthenticated().user._id === post.postedBy._id && (
                <>
                  <Link
                    ...
                    className="btn btn-raised btn-info ml-4"
                  >
                    Update Post
                  </Link>
                  <button
                    onClick={this.confirmDelete}
                    className="btn btn-raised btn-warning ml-4"
                  >
                    Delete Post
                  </button>
                </>
