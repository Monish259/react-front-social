...
class Comment extends Component {
  ...
  isValid() {
    ...
    if (text.length === 0 || text.length > 150) {
      this.setState({
        error: "Cannot post empty comment or more than 150 cahracters long!!",
      });
      return false;
    }
  ...
  addComment = (event) => {
    ...
    if (this.isValid()) {
      comment(postId, userId, token, upload).then((data) => {
        if (data.error) console.log(data.error);
        else {
          this.setState({ text: "", error: "", loadingComment: false });
          this.props.updateComments(data.comments);
        }
      });
    } else this.setState({ loadingComment: false });
  };
  ...
  removeComment = (comment) => {
    ...
    unComment(postId, userId, token, comment).then((data) => {
      if (data.error) console.log(data.error);
  ...
  confirmDelete = (comment) => {
    //postId passed in component form profile.js file is present in props here
  ...
  render() {
    ...
    const { error, redirectToSignin, loadingComment } = this.state;
      ...
      <div>
        ...
        <div className="form-group">
          <form onSubmit={this.addComment}>
            ...
            />
            {loadingComment ? (
              <div className="btn btn-raised btn-secondary mt-2">
                ...
                <div>
                  <Link to={`/user/${comment.postedBy._id}`}>
                    <img
                      ...
                      onError={(photo) => (photo.target.src = `${avatar}`)}
                    ...
                    <p className="font-italic mark mb-5">
                      ...
                      {isAuthenticated() &&
                        isAuthenticated().user._id === comment.postedBy._id && (
                          ...
                          >
                            {/* passing comment in removeComment in a arrow function so that it dont runs on every load unless is clicked*/}
